...
def test_get_songs_by_country_id(mocker, mock_session, song_entity, song_model):
    
    mocker.patch.object(SongRepository, 'get_songs_by_country_id', return_value=[song_entity])
    mocker.patch.object(SongModelMapper, 'map_to_song_model_without_submodels', return_value=song_model)
    
    country_id = 1
    result = SongService(mock_session).get_songs_by_country_id(country_id=country_id)
...
def test_update_song(mocker, mock_session, song_model, song_entity, country_entity, event_entity):
    ...
    mocker.patch.object(SongRepository, 'check_is_song_participating_in_a_ceremony', return_value=False)
    ...
    SongRepository(mock_session).update_song.assert_called_once()
...
def test_update_song_participating_in_ceremony(mocker, mock_session, song_model, song_entity, country_entity, event_entity):

    mocker.patch.object(SongRepository, 'get_song', return_value=song_entity)
    mocker.patch.object(SongRepository, 'check_is_song_participating_in_a_ceremony', return_value=True)
    mocker.patch.object(SongModelMapper, 'map_to_song_entity', return_value=song_entity)
    mocker.patch.object(CountryRepository, 'get_country', return_value=country_entity)
    mocker.patch.object(EventRepository, 'get_event', return_value=event_entity)
    mocker.patch.object(SongRepository, 'get_song_by_country_and_event_id', return_value=None)
    mocker.patch.object(SongRepository, 'check_existing_song_marked_as_belongs_to_host_country', return_value=None)
    mocker.patch.object(SongRepository, 'update_song')
    ...
    SongRepository(mock_session).check_is_song_participating_in_a_ceremony.assert_called_once_with(song_id=song_id)
...
def test_delete_song(mocker, mock_session, song_model, song_entity):
    ...
    SongRepository(mock_session).delete_song.assert_called_once()
...
def test_delete_song_participating_in_ceremony(mocker, mock_session, song_model, song_entity):
    ...
    SongRepository(mock_session).check_is_song_participating_in_a_ceremony.assert_called_once()
    SongRepository(mock_session).delete_song.assert_not_called()

@pytest.mark.parametrize("position", [1, 4, 6, 11, 16, 21, 28])
def test_calculate_potential_scores(position: int):
    result = SongService(None).calculate_potential_scores(position=position)
